...
public class StockCanvas {
	...
	public static Tab multiChart() throws SQLException {
		symbol = " ";
		...
		lineChart.setPrefSize( 1091, 450 );
		barChart.setPrefSize( 1091, 311 );
	...
	private static class AddToQueue implements Runnable {
		...
		private static BigDecimal volume;
		private static StockQuote stockQuote;
		@Override
		public void run() {
			try {
				Quote stockQuote =
					   IEX.initiateIEX().executeRequest(
					   	   new QuoteRequestBuilder().withSymbol( symbol ).build() );
				
				last = stockQuote.getLatestPrice();
				volume = stockQuote.getIexVolume();
				queue1.add( last );
				queue2.add( volume );
